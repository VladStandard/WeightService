-- Nomenclature. Master records include.
declare @id int = -2147424979
use [VSDWH]
select 
	 [n].[ID]
	,[n].[Code]
...
where [n].[IsProduct] = 1 and [n].[MasterId] = @id
	and [n].[MasterId] <> [n].[Id] and [n].[InformationSystemID] <> 7
order by [n].[Name]
--select * from [ETL].[InformationSystems]