...
public sealed partial class WsControllerHelper
{
    ...
    /// <returns></returns>
    private bool UpdateItemDb<T>(WsResponse1cShortModel response, Guid importUid, T itemXml, T? itemDb, bool isCounter) where T : ISqlTable
    {
    ...
    /// <param name="isCounter"></param>
    /// <returns></returns>
    private bool UpdateItem1cDb<T>(WsResponse1cShortModel response, Guid importUid, T itemXml, T? itemDb, bool isCounter) where T : ISqlTable1c
    {
        if (itemDb is null || itemDb.IsNew) return false;
        itemDb.UpdateProperties(itemXml);
        SqlCrudResultModel dbUpdate = DataContext.DataAccess.UpdateForce(itemDb);
    ...
    /// <param name="isCounter"></param>
    private bool SaveItemDb<T>(WsResponse1cShortModel response, Guid importUid, T item, bool isCounter) where T : ISqlTable
    {
