-- Linked server SQLSRSP01 - CREATIO
-- Connect from SQLSRSP01\LEEDS
USE [master]
declare @server_name nvarchar(255) = N'CREATIO\INS1'
...
EXEC master.dbo.sp_addlinkedsrvlogin @rmtsrvname=@server_name,
	@useself=N'False', @locallogin=N'KOLBASA-VS\Morozov_DV', @rmtuser=N'LINK_CREATIO', @rmtpassword='Qwert!@#$%'
...
GO
SELECT * FROM OPENQUERY([CREATIO\INS1], 'SELECT [Id],[Name],[IP],[MAC],[IdRRef],[CreateDate],[ModifiedDate],[Marked] FROM [SCALES].[db_scales].[Hosts]')
