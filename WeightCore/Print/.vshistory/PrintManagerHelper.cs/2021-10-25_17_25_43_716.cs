...
namespace WeightCore.Print
{
    public class PrintManagerHelper
    {
        ...
        public void OpenJob(bool isTscPrinter, TscPrintControlHelper.Callback callbackAfterJob)
        {
            Con?.Open();
            if (isTscPrinter)
                OpenTsc(callbackAfterJob);
            else
                OpenZebra();
        }
        ...
        public void OpenTsc(TscPrintControlHelper.Callback callbackAfterJob)
        {
            UserLabelCount = 1;
            try
            {
                lock (_locker)
                {
                    if (!PrintCmdQueue.IsEmpty)
                    {
                        if (PrintCmdQueue.TryDequeue(out string request))
                        {
                            ...
                            {
                                TscPrintControl.CmdSendCustom(request, callbackAfterJob);
                            }
                        }
                    }
                }
