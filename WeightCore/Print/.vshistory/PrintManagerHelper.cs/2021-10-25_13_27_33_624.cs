...
namespace WeightCore.Print
{
    public class PrintManagerEntity
    {
        ...
        public TscPrintControlHelper PrintControl = TscPrintControlHelper.Instance;
        ...
        public PrintManagerEntity(Connection connection, string name, string ip, int port,
        ...
        {
            Con = connection;
            PrintControl.Init(name, ip, port);
        }

        public PrintManagerEntity(string name, string ip, int port, int waitWhileMiliSeconds, int waitExceptionMiliSeconds, int waitCloseMiliSeconds) :
            this(waitWhileMiliSeconds, waitExceptionMiliSeconds, waitCloseMiliSeconds)
        {
                ...
                lock (_locker)
                {
                    if (!PrintCmdQueue.IsEmpty)
                    {
                        if (PrintCmdQueue.TryDequeue(out string request))
                        {
                            ...
                            {
                                PrintControl.CmdSendCustom(request);
                            }
                        }
                    }
                }
