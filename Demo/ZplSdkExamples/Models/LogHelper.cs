    ...
    private void Message(string message, EnumLogType logType, [CallerFilePath] string sourceFilePath = "", [CallerMemberName] string memberName = "", [CallerLineNumber] int sourceLineNumber = 0)
    {
        ...
        switch (logType)
        {
            case EnumLogType.Debug:
                MdInvokeTextBox.AddTextFormat(_textBox, dt, "Debug. " + message);
                isDebug = true;
                break;
            case EnumLogType.Error:
                MdInvokeTextBox.AddTextFormat(_textBox, dt, "Error. " + message);
                isDebug = true;
                break;
            case EnumLogType.Fatal:
                MdInvokeTextBox.AddTextFormat(_textBox, dt, "Fatal. " + message);
                isDebug = true;
                break;
            case EnumLogType.Info:
                MdInvokeTextBox.AddTextFormat(_textBox, dt, "Info. " + message);
                break;
        ...
        if (isDebug)
        {
            ...
            MdInvokeTextBox.AddTextFormat(_textBox, dt, msgDebug);
        }
    }
