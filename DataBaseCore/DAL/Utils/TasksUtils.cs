    ...
    public static class TasksUtils
    {
        ...
        public static void SaveTask(TaskEntity task, TaskTypeEntity taskType, int scaleId, bool enabled)
        {
            using (SqlConnection con = SqlConnectFactory.GetConnection())
            {
                ...
                if (task == null)
                {
                    using (SqlCommand cmd = new SqlCommand(SqlQueries.Tasks.InsertTask))
                    {
                        cmd.Connection = con;
                ...
                {
                    using (SqlCommand cmd = new SqlCommand(SqlQueries.Tasks.UpdateTask))
                    {
                        cmd.Connection = con;
                        cmd.Parameters.Clear();
        ...
        public static Guid GetTaskUid(string taskName)
        {
            ...
            using (SqlConnection con = SqlConnectFactory.GetConnection())
            {
                ...
                using (SqlCommand cmd = new SqlCommand(SqlQueries.Tasks.GetTaskUid))
                {
                    cmd.Connection = con;
                    cmd.Parameters.Clear();
        ...
        public static TaskEntity GetTask(Guid taskTypeUid, int scaleId)
        {
            ...
            {
                con.Open();
                using (SqlCommand cmd = new SqlCommand(SqlQueries.Tasks.GetTaskByTypeAndScale))
                {
                    cmd.Connection = con;
                    cmd.Parameters.Clear();
        ...
        public static TaskEntity GetTask(Guid taskUid)
        {
            ...
            {
                con.Open();
                using (SqlCommand cmd = new SqlCommand(SqlQueries.Tasks.GetTaskByUid))
                {
                    cmd.Connection = con;
                    cmd.Parameters.Clear();
