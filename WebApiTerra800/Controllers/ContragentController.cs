    ...
    public class ContragentController : ApiController
    {
        ...
        public HttpResponseMessage Get(DateTime StartDate, DateTime EndDate, int Offset = 0, int RowCount = 10)
        {
            ...
            using (var conn = SqlHelper.GetConnection())
            {
                ...
                using (var cmd = SqlHelper.GetCommand("SELECT [IIS].[fnGetContragentChangesList] (@StartDate, @EndDate, @Offset, @RowCount)", conn, sqlParameters))
                {
                    try
                    {
        ...
        public HttpResponseMessage Get(int id)
        {
            ...
            using (var conn = SqlHelper.GetConnection())
            {
                ...
                using (var cmd = SqlHelper.GetCommand("SELECT [IIS].[fnGetContragentByID] (@ID)", conn, sqlParameters))
                {
                    try
                    {
                        using (var xmlReader = cmd.ExecuteXmlReader())
