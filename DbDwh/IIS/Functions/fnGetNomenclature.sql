-- [IIS].[fnGetNomenclature]

-- DROP FUNCTION
DROP FUNCTION IF EXISTS [IIS].[fnGetNomenclature]
GO

-- CREATE FUNCTION
CREATE FUNCTION [IIS].[fnGetNomenclature]
(@code NVARCHAR(255) = NULL, @id BIGINT = NULL)
RETURNS XML
BEGIN
	-- DECLARE.
	...
	DECLARE @xml XML = (
		...
		LEFT JOIN @tableNomenclatureTypes [NT] ON [N].NomenclatureType = [NT].[CodeInIS] --AND Nomenclature.[InformationSystemID] = t.[InformationSystemID]
		...
		WHERE
			...
			AND (  -- SWITCH @id OR @code
				(@id IS NOT NULL AND [N].[ID] = @id) OR 
				(@code IS NOT NULL AND [N].[Code] = @code)
			)
	...
	IF (@code IS NOT NULL) BEGIN
		SET @api = '/api/nomenclature/?code=' + @code;
	END ELSE IF (@id IS NOT NULL) BEGIN
		SET @api = '/api/nomenclature/?id=' + CAST(@id AS NVARCHAR(100));
	END
...
-- ACCESS
GRANT EXECUTE ON [IIS].[fnGetNomenclature] TO [TerraSoftRole]
GO

-- CHECK FUNCTION
DECLARE @id BIGINT = -2147460739
SELECT [IIS].[fnGetNomenclature] (DEFAULT, @id) [fnGetNomenclature]
DECLARE @code NVARCHAR(255) = N'ЦБД00018851'
SELECT [IIS].[fnGetNomenclature] (@code, DEFAULT) [fnGetNomenclature]
