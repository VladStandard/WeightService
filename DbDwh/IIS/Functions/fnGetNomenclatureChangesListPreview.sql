...
AS BEGIN
	...
	ELSE BEGIN
		...
		SET @xml = (SELECT * FROM (SELECT
			...
			AS XML) [Prices]
			FROM [DW].[DimNomenclatures] [N]
			LEFT JOIN @tableNomenclatureTypes [NT] ON [N].NomenclatureType = [NT].[CodeInIS] --AND [n].[InformationSystemID] = [T].[InformationSystemID]
			LEFT JOIN @tableCurrentPlannedCosts [vCPC] ON [N].[ID] = [vCPC].[NomenclatureID]
			LEFT JOIN @tableSalfeCosts [vSC] ON [N].[ID]=[vSC].[NomenclatureId]
