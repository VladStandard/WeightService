...
-- DROP FUNCTION
DROP FUNCTION IF EXISTS [IIS].[fnGetNomenclatureChangesList_Preview]
...
AS BEGIN
	...
	END ELSE BEGIN
		...
		DECLARE @Api NVARCHAR(1024) = '/api/nomenclatures/?StartDate=' + CONVERT(NVARCHAR(255), @StartDate, 126) + '&EndDate=' + CONVERT(NVARCHAR(255), @EndDate, 126) + 
			'&Offset=' + CAST(@Offset AS NVARCHAR(100)) + '&RowCount=' + CAST(@RowCount AS NVARCHAR(100))
		DECLARE @ApiPreview NVARCHAR(1024) = '/api/nomenclatures_preview/?StartDate=' + CONVERT(NVARCHAR(255), @StartDate, 126) + '&EndDate=' + CONVERT(NVARCHAR(255), @EndDate, 126) + 
			'&Offset=' + CAST(@Offset AS NVARCHAR(100)) + '&RowCount=' + CAST(@RowCount AS NVARCHAR(100))
