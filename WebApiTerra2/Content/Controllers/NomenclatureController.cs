...
namespace terra.Controllers
{
    public class NomenclatureController : ApiController
    {
        ...
        public HttpResponseMessage Get(DateTime StartDate, DateTime EndDate, int Offset = 0, int RowCount = 10)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT [IIS].[fnGetNomenclatureChangesList] (@StartDate, @EndDate, @Offset, @RowCount)", conn))
                {
                    try
                    {
        ...
        public HttpResponseMessage Get(int id)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT [IIS].[fnGetNomenclatureByID] (@ID)", conn))
                {
                    try
                    {
