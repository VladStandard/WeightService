...
namespace terra.Controllers
{
    public class ShipmentController : ApiController
    {
        ...
        public HttpResponseMessage Get(DateTime StartDate, DateTime EndDate, int Offset = 0, int RowCount = 10)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT [IIS].[fnGetShipmentChangesList] (@StartDate, @EndDate, @Offset, @RowCount)", conn))
                {
                    try
                    {
        ...
        public HttpResponseMessage Get(Int64 id)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT [IIS].[fnGetShipmentByID] (@ID)", conn))
                {
                    try
                    {
