...
namespace ScalesUI.Forms
{
    public partial class MainForm : Form
    {
        ...
        private void CallbackPrintManager()
        {
            ...
            if (_sessionState.IsTscPrinter)
            {
                MDSoft.WinFormsUtils.InvokeControl.SetText(fieldPrintManager, _sessionState.TaskManager.PrintManager.TscPrintControl.DriverStatus
                    ? $"Принтер {_sessionState.TaskManager.PrintManager.TscPrintControl.IpAddress}: доступен | {ch}" 
                    : $"Принтер {_sessionState.TaskManager.PrintManager.TscPrintControl.IpAddress}: недоступен | {ch}");
            }
            // Zebra printers.
            else
            {
                ...
                if (_sessionState.TaskManager.PrintManager.CurrentStatus != null)
                {
                    MDSoft.WinFormsUtils.InvokeControl.SetText(fieldPrintManager, _sessionState.TaskManager.PrintManager.CurrentStatus.isReadyToPrint
                        ? $"Принтер {_sessionState.TaskManager.PrintManager.ZebraPrinter.IpAddress}: доступен | {ch}" 
                        : $"Принтер {_sessionState.TaskManager.PrintManager.ZebraPrinter.IpAddress}: недоступен | {ch}");
                }
            }
        ...
        private void ButtonPrint_Click(object sender, EventArgs e)
        {
            try
            {
                _sessionState.TaskManager.OpenPrintManager(CallbackPrintManager, CallbackPrintManagerClose,
