...
^FB300,1,0,C,0
^FH^FD</xsl:text>
<xsl:value-of select="$netWeightKg2Dot3Eng"/>
<xsl:text>
^FS
</xsl:text>
...
^FH^FD</xsl:text>
<!-- плу -->
<xsl:variable name="plu" select="/WeighingFactEntity/PLU/EAN13"/>
<xsl:value-of select="substring(concat('0000000',$plu),string-length($plu)+1,7)"/>
<!-- Вес [5 симв] -->
<xsl:value-of select="$netWeightKg2"/>
<xsl:value-of select="$netWeightGr3"/>
<xsl:text>^FS
</xsl:text>
