...
{
    [TestFixture]
    internal class OrderEntityTests
    {
        [Test]
        ...
        public void Entity_Crud_DoesNotThrow()
        {
            Assert.DoesNotThrow(() =>
            {
                ...
                {
                    List<BaseEntity>? items = TestsUtils.DataAccess.Crud.GetEntities<OrderEntity>(
                    ...
                    if (items != null)
                    {
                        List<OrderEntity> itemsCast = items.Select(x => (OrderEntity)x).ToList();
                        if (itemsCast.Count > 0)
                        {
                            foreach (OrderEntity item in itemsCast)
                            {
                                OrderEntity itemCopy = item.CloneCast;
                                Assert.AreEqual(true, item.Equals(itemCopy));
                                Assert.AreEqual(true, itemCopy.Equals(item));
                                OrderEntity itemChange = new()
                                {
                                    IsMarked = true,
                                };
