...
{
    [TestFixture]
    internal class WorkShopEntityTests
    {
        [Test]
        ...
        public void Entity_Crud_DoesNotThrow()
        {
            Assert.DoesNotThrow(() =>
            {
                ...
                {
                    List<BaseEntity>? items = TestsUtils.DataAccess.Crud.GetEntities<WorkShopEntity>(
                    ...
                    if (items != null)
                    {
                        List<WorkShopEntity> itemsCast = items.Select(x => (WorkShopEntity)x).ToList();
                        if (itemsCast.Count > 0)
                        {
                            foreach (WorkShopEntity item in itemsCast)
                            {
                                WorkShopEntity itemCopy = item.CloneCast();
                                Assert.AreEqual(true, item.Equals(itemCopy));
                                Assert.AreEqual(true, itemCopy.Equals(item));
                                WorkShopEntity itemChange = new()
                                {
                                    IsMarked = true,
                                };
