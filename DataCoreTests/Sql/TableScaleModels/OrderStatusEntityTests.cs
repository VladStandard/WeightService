    ...
    public void Entity_Crud_DoesNotThrow()
    {
        Assert.DoesNotThrow(() =>
        {
            foreach (bool isShowMarkedItems in TestsEnums.GetBool())
            {
                ...
                if (items != null)
                {
                    List<OrderStatusEntity> itemsCast = items.Select(x => (OrderStatusEntity)x).ToList();
                    if (itemsCast.Count > 0)
                    {
                        foreach (OrderStatusEntity item in itemsCast)
                        {
                            OrderStatusEntity itemCopy = item.CloneCast();
                            Assert.AreEqual(true, item.Equals(itemCopy));
                            Assert.AreEqual(true, itemCopy.Equals(item));
                            OrderStatusEntity itemChange = new()
                            {
                                IsMarked = true,
                            };
