...
{
    [TestFixture]
    internal class TaskEntityTests
    {
        [Test]
        ...
        public void Entity_Crud_DoesNotThrow()
        {
            Assert.DoesNotThrow(() =>
            {
                ...
                {
                    List<BaseEntity>? items = TestsUtils.DataAccess.Crud.GetEntities<TaskEntity>(
                    ...
                    if (items != null)
                    {
                        List<TaskEntity> itemsCast = items.Select(x => (TaskEntity)x).ToList();
                        if (itemsCast.Count > 0)
                        {
                            foreach (TaskEntity item in itemsCast)
                            {
                                TaskEntity itemCopy = item.CloneCast();
                                Assert.AreEqual(true, item.Equals(itemCopy));
                                Assert.AreEqual(true, itemCopy.Equals(item));
                                TaskEntity itemChange = new()
                                {
                                    IsMarked = true,
                                };
