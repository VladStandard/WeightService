...
{
    [TestFixture]
    internal class ProductSeriesEntityTests
    {
        [Test]
        ...
        public void Entity_Crud_DoesNotThrow()
        {
            Assert.DoesNotThrow(() =>
            {
                foreach (bool isShowMarkedItems in TestsEnums.GetBool())
                {
                    ...
                    if (items != null)
                    {
                        List<ProductSeriesEntity> itemsCast = items.Select(x => (ProductSeriesEntity)x).ToList();
                        if (itemsCast.Count > 0)
                        {
                            foreach (ProductSeriesEntity item in itemsCast)
                            {
                                ProductSeriesEntity itemCopy = item.CloneCast();
                                Assert.AreEqual(true, item.Equals(itemCopy));
                                Assert.AreEqual(true, itemCopy.Equals(item));
                                ProductSeriesEntity itemChange = new()
                                {
                                    IsMarked = true,
                                };
