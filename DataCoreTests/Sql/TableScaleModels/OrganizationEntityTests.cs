...
{
    [TestFixture]
    internal class OrganizationEntityTests
    {
        [Test]
        ...
        public void Entity_Crud_DoesNotThrow()
        {
            Assert.DoesNotThrow(() =>
            {
                ...
                {
                    List<BaseEntity>? items = TestsUtils.DataAccess.Crud.GetEntities<OrganizationEntity>(
                    ...
                    if (items != null)
                    {
                        List<OrganizationEntity> itemsCast = items.Select(x => (OrganizationEntity)x).ToList();
                        if (itemsCast.Count > 0)
                        {
                            foreach (OrganizationEntity item in itemsCast)
                            {
                                OrganizationEntity itemCopy = item.CloneCast;
                                Assert.AreEqual(true, item.Equals(itemCopy));
                                Assert.AreEqual(true, itemCopy.Equals(item));
                                OrganizationEntity itemChange = new()
                                {
                                    IsMarked = true,
                                };
