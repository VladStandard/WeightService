...
{
    [TestFixture]
    internal class TaskTypeEntityTests
    {
        [Test]
        ...
        public void Entity_Crud_DoesNotThrow()
        {
            Assert.DoesNotThrow(() =>
            {
                ...
                {
                    List<BaseEntity>? items = TestsUtils.DataAccess.Crud.GetEntities<TaskTypeEntity>(
                    ...
                    if (items != null)
                    {
                        List<TaskTypeEntity> itemsCast = items.Select(x => (TaskTypeEntity)x).ToList();
                        if (itemsCast.Count > 0)
                        {
                            foreach (TaskTypeEntity item in itemsCast)
                            {
                                TaskTypeEntity itemCopy = item.CloneCast();
                                Assert.AreEqual(true, item.Equals(itemCopy));
                                Assert.AreEqual(true, itemCopy.Equals(item));
                                TaskTypeEntity itemChange = new()
                                {
                                    IsMarked = true,
                                };
