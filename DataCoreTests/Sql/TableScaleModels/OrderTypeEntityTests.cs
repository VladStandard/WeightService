...
[TestFixture]
internal class OrderTypeEntityTests
{
    [Test]
    ...
    public void Entity_Crud_DoesNotThrow()
    {
        Assert.DoesNotThrow(() =>
        {
            ...
            {
                List<BaseEntity>? items = TestsUtils.DataAccess.Crud.GetEntities<OrderTypeEntity>(
                ...
                if (items != null)
                {
                    List<OrderTypeEntity> itemsCast = items.Select(x => (OrderTypeEntity)x).ToList();
                    if (itemsCast.Count > 0)
                    {
                        foreach (OrderTypeEntity item in itemsCast)
                        {
                            OrderTypeEntity itemCopy = item.CloneCast();
                            Assert.AreEqual(true, item.Equals(itemCopy));
                            Assert.AreEqual(true, itemCopy.Equals(item));
                            OrderTypeEntity itemChange = new()
                            {
                                IsMarked = true,
                            };
