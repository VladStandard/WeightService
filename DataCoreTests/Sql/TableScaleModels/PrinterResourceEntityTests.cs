...
{
    [TestFixture]
    internal class PrinterResourceEntityTests
    {
        [Test]
        ...
        public void Entity_Crud_DoesNotThrow()
        {
            Assert.DoesNotThrow(() =>
            {
                ...
                {
                    List<BaseEntity>? items = TestsUtils.DataAccess.Crud.GetEntities<PrinterResourceEntity>(
                    ...
                    if (items != null)
                    {
                        List<PrinterResourceEntity> itemsCast = items.Select(x => (PrinterResourceEntity)x).ToList();
