...
[TestFixture]
internal class TemplateEntityTests
{
    [Test]
    ...
    public void Entity_Crud_DoesNotThrow()
    {
        Assert.DoesNotThrow(() =>
        {
            ...
            {
                List<BaseEntity>? items = TestsUtils.DataAccess.Crud.GetEntities<TemplateEntity>(
                ...
                if (items != null)
                {
                    List<TemplateEntity> itemsCast = items.Select(x => (TemplateEntity)x).ToList();
                    if (itemsCast.Count > 0)
                    {
                        foreach (TemplateEntity item in itemsCast)
                        {
                            TemplateEntity itemCopy = item.CloneCast();
                            Assert.AreEqual(true, item.Equals(itemCopy));
                            Assert.AreEqual(true, itemCopy.Equals(item));
                            TemplateEntity itemChange = new()
                            {
                                IsMarked = true,
                            };
