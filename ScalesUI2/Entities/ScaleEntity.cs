using ScalesLib.Entities;
using ScalesLib.Helpers;
using ScalesLib.Sql.Entities;
...
namespace ScalesUI.Entities
{
    ...
    public class ScaleEntity
    {
        ...
        private string UpdateQuery()
        {
            ...
            //    @{RRefID.Name}, @{Description.Name}, @{DeviceIP.Name}, @{DevicePort.Name}, @{DeviceMAC.Name}, 
            //    @{DeviceSendTimeout.Name}, @{DeviceReceiveTimeout.Name}, @{DeviceComPort.Name}, 
            //    @{ZebraIP.Name}, @{ZebraPort.Name}, @{UseOrder.Name}, @{VerScalesUI.Name}
            //    ,@ID OUTPUT
            //                ".TrimStart('\r', ' ', '\n').TrimEnd('\r', ' ', '\n');
            return $@"
DECLARE @ID int
EXECUTE [db_scales].[SetNewScale] 
    @Uuid, @{nameof(Description)}, @IP, @Port, @{nameof(DeviceMAC)}, 
    @{nameof(DeviceSendTimeout)}, @{nameof(DeviceReceiveTimeout)}, @{nameof(DeviceComPort)}, 
    @{nameof(ZebraIP)}, @{nameof(ZebraPort)}, @{nameof(UseOrder)}, @{nameof(VerScalesUI)}, @DeviceNumber
    ,@ID OUTPUT
                ".TrimStart('\r', ' ', '\n').TrimEnd('\r', ' ', '\n');
        }
