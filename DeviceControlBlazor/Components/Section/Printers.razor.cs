...
namespace DeviceControlBlazor.Components.Section
{
    public partial class Printers
    {
        ...
        private string GetHeader() => LocalizationStrings.TableTitlePrinters;
        ...
        private async Task ActionEditAsync(EnumTable table, BaseIdEntity item, BaseIdEntity parentEntity)
        {
            ...
            await RunTasks(LocalizationStrings.GetItemTitle(table), "", LocalizationStrings.DialogResultFail, "",
                new List<Task> {
                    new(() => {
                        ActionAsync(table, EnumTableAction.Edit, item, LocalizationStrings.UriRouteItemPrinter, false)
                            .ConfigureAwait(true);
                    }),
