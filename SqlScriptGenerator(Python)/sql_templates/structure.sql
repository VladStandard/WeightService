	...
	BEGIN TRAN
		...
		END ELSE BEGIN
			...
			IF EXISTS (SELECT 1 FROM [sys].[tables] WHERE [name] = N'{table_name}') BEGIN
				...
				END;
				IF NOT EXISTS (SELECT 1 FROM [SYS].[DEFAULT_CONSTRAINTS] WHERE [NAME] = 'DF_{table_name}_NAME') BEGIN
					ALTER TABLE [db_scales].[{table_name}] ADD CONSTRAINT [DF_{table_name}_NAME] DEFAULT ('') FOR [NAME];
					PRINT N'[i] CONSTRAINT [DF_{table_name}_NAME] WAS CREATED';
				END;
				IF NOT EXISTS (SELECT 1 FROM [SYS].[DEFAULT_CONSTRAINTS] WHERE [NAME] = 'DF_{table_name}_DESCRIPTION') BEGIN
					ALTER TABLE [db_scales].[{table_name}] ADD CONSTRAINT [DF_{table_name}_DESCRIPTION] DEFAULT ('') FOR [DESCRIPTION];
					PRINT N'[i] CONSTRAINT [DF_{table_name}_DESCRIPTION] WAS CREATED';
				END;
			END;
		END;
        ...
        IF (@IS_COMMIT = 1) BEGIN
            COMMIT TRAN;
            PRINT N'[i] JOB WAS COMMITED';
        END ELSE BEGIN
            ROLLBACK TRAN;
            PRINT N'[!] JOB WAS ROLLBACKED';
        END;
END;
------------------------------------------------------------------------------------------------------------------------