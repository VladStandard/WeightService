...
{
    public PluEntity GetByUid(Guid uid) => Session.Get<PluEntity>(uid);

    public PluEntity GetByUid1C(Guid uid1C) =>
        Session.Query<PluEntity>().FirstOrDefault(i => i.Uid1C == uid1C) ?? new();
    ...
    public IEnumerable<PluEntity> GetListByQuery(QueryOver<PluEntity> query) =>
        query.DetachedCriteria.GetExecutableCriteria(Session).List<PluEntity>().OrderBy(i => i.Number);
}