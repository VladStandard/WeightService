...
public sealed partial class Badge : ComponentBase
{
    ...
    private string BadgeClasses => Css.Class("inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs",
                                             "font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring",
                                             "focus:ring-offset-2", VariantClasses, Class);
    ...
    private string VariantClasses => Variant switch
    {
        ...
        BadgeVariant.Destructive => "border-transparent bg-secondary text-secondary-foreground hover:bg-secondary/80",
        BadgeVariant.Outline => "border border-input bg-background hover:bg-accent hover:text-accent-foreground",
        BadgeVariant.Secondary => "text-foreground",
        _ => string.Empty
    };
}
