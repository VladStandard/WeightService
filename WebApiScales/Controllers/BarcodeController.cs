...
public class BarCodeController : WebControllerBase
{
        ...
        [FromQuery(Name = "format")] string format = "", [FromHeader(Name = "host")] string host = "")
    {
        ...
        ControllerHelp.LogRequest(nameof(WebApiScales), dtStamp, barcode, format, host, string.Empty).ConfigureAwait(false);
    ...
    {
        DateTime dtStamp = DateTime.Now;
        ControllerHelp.LogRequest(nameof(WebApiScales), dtStamp, query, format, host, string.Empty).ConfigureAwait(false);
        ContentResult result = ControllerHelp.GetContentResult(() =>
    ...
    {
        DateTime dtStamp = DateTime.Now;
        ControllerHelp.LogRequest(nameof(WebApiScales), dtStamp, barcodeTop, format, host, string.Empty).ConfigureAwait(false);
        ContentResult result = ControllerHelp.GetContentResult(() =>
            ControllerHelp.NewResponse1cFromQuery(SessionFactory, SqlQueriesBarcodes.FindTop,
