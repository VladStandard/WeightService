...
using PluBundleFkValidator = WsStorageCore.TableScaleFkModels.PlusBundlesFks.PluBundleFkValidator;
...
public sealed class WsDataContextModel
{
    ...
    public WsDataAccessHelper DataAccess { get; set; } = WsDataAccessHelper.Instance;
    ...
    public DeviceModel GetItemDeviceOrCreateNew(string name)
    {
        ...
        DeviceModel device = DataAccess.GetItemNotNullable<DeviceModel>(sqlCrudConfig);
        if (device.IsNew)
        {
            device = new()
            {
        ...
        DataAccess.SaveOrUpdate(device);
        return device;
    }
    ...
    public DeviceModel GetItemDeviceNotNullable(string name)
    {
        SqlCrudConfigModel sqlCrudConfig = WsSqlCrudConfigUtils.GetCrudConfig(
            nameof(WsSqlTableBase.Name), name, true, false);
        return DataAccess.GetItemNotNullable<DeviceModel>(sqlCrudConfig);
    }
